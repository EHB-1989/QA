...
def init_db():
    ...
    with get_db_connection() as conn:
        ...
        if conn.execute("SELECT * FROM livres").fetchone() is None:
            conn.executemany(
                "INSERT INTO livres (titre, auteur, est_emprunte) VALUES (?, ?, ?)",
                livres_default,
            )
...
def emprunter_livre_db(titre):
    with get_db_connection() as conn:
        livre = conn.execute(
            "SELECT * FROM livres WHERE titre = ? AND est_emprunte = 0", (titre,)
        ).fetchone()
        if livre:
...
def retourner_livre_db(titre):
    with get_db_connection() as conn:
        livre = conn.execute(
            "SELECT * FROM livres WHERE titre = ? AND est_emprunte = 1", (titre,)
        ).fetchone()
        if livre:
