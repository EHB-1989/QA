...
def init_db():
    ...
    if Product.query.count() == 0:
        products = [
            ...
            Product(
                name="Smartphone",
                description="A personal device that combines cellular and mobile computing functions",
            ),
            Product(
                name="Headphones",
                description="A pair of small loudspeaker drivers worn on or around the head over a user's ears",
            ),
            Product(
                name="Charger", description="A device used to charge electronic devices"
            ),
